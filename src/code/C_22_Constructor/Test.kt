...
class TestClass(var name:String, var age:Int) {
    fun introduce() {
        println("Hi I'm ${name} and ${age} years old.")
    }
}

class TestRestController(private val serviceFactory: TestClass) { // var을 붙이면 생성자의 매개변수를
    fun doSomething(): TestClass {
        return serviceFactory // 클래스내부에서도 쓸 수 있다
    }
}