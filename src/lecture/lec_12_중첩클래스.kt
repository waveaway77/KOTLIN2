...
Sealed class : 상속이 가능하도록 추상클래스를 만들기는 하지만 외부에서는 이 클래스를 상속 받지 못하게 하고 싶다. 이 경우 하위 클래스를 봉인한다.
컴파일 시에 하위 클래스의 타입을 모두 기억하므로 런타임 때 클래스 타입이 추가 될 수 없게 된다
===> sealed class는 컴파일 단계에서 sealed class의 자식 클래스가 어떤 것이 있는지 모두 파악하고 있다.
